defmodule QuoteGPTest do
  ...
  test "generates code" do
    assert QuoteGP.Generation.code_tree(20) !== nil
  end

  test "mutates code" do
    assert QuoteGP.GeneticOperators.mutation(QuoteGP.Generation.code_tree(20), 0.1) !== nil
  end

  test "crosses over code" do
    assert QuoteGP.GeneticOperators.crossover(QuoteGP.Generation.code_tree(20), QuoteGP.Generation.code_tree(20)) !== nil
  end

  test "runs code with proper binding" do
    assert Code.eval_quoted(QuoteGP.Generation.code_tree(20), [input: 10]) !== nil
  end
  ...
  test "tournament select" do
    QuoteGP.Population.build(3)
    |> QuoteGP.Population.evaluate([{ 1, 2 }, { 2, 4}])
    |> QuoteGP.Population.tournament(3)
  end

  test "runs a generation" do
    cases = (1..5) |> Enum.map(fn x -> {x, x * x + 3 * x + 8} end)

    QuoteGP.Population.build(1000)
